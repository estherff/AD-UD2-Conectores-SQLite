 ...
 * en el proyecto Java.
 * 
 * El patrón Singleton se construye con un constructor privado que es llamado por
 * un método publico y estático (getConexionSingleton) que analiza si ya está 
 * creada una instancia de la clase, en cuyo caso la devuelve y si no está creada,
 * la crea.
 * De esta forma, cada vez que se necesita abrir una conexión de la base de datos
 * se puede llamar al método getConexionSingleton con la garantía de que no duplicará
 * las conexiones y siempre nos devolverá la conexión abierta o abrirá una nueva si 
 * es necesario.
 *
